  ...
  const toggleCall = async () => {
    if (callActive) vapi.stop();
      ...
      try {
        ...
        if (!assistantId || assistantId === 'your_vapi_assistant_id') {
          setMessages([{
            role: 'assistant',
            content: 'Vapi Assistant ID is not configured yet. Please set up your Vapi account and add the assistant ID to your environment variables.'
          }]);
          setConnecting(false);
          setCallEnded(true);
          return;
        ...
        setMessages([{
          role: 'assistant',
          content: `Failed to start call: ${error.message || 'Please check your Vapi configuration.'}`
        }]);
        setConnecting(false);
        setCallEnded(true);
      }
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        ...
        <Card className="bg-card/90 backdrop-blur-sm border border-border overflow-hidden relative">
          <div className="aspect-video flex flex-col items-center justify-center p-6 relative">
            ...
            <h2 className="text-xl font-bold text-foreground">AIvory AI</h2>
            <p className="text-sm text-muted-foreground mt-1">Dental Assistant</p>            {/* SPEAKING INDICATOR */}
            <div
          ...
          <div className="space-y-3">
            {messages.map((msg, index) => (
              <div key={index} className="message-item animate-in fade-in duration-300">
                <div className="font-semibold text-xs text-muted-foreground mb-1">
                  {msg.role === "assistant" ? "AIvory AI" : "You"}:
                </div>
            ...
            {callEnded && (
              <div className="message-item animate-in fade-in duration-300">
                ...
                <p className="text-foreground">Call ended. Thank you for using AIvory AI!</p>
              </div>
            )}
          </div>
        </div>
